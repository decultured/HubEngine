	...
	public class BrickBreakerGame 
	{
		...
		public function Update(elapsedTime:Number):void
		{
			...
			for (var i:uint = 0; i < blocksLength; i++) {
				...
				if (_Ball.Position.x > _Blocks[i].Position.x && _Ball.Position.x < _Blocks[i].Position.x + _Blocks[i].Width) {
					if (_Ball.Position.y > _Blocks[i].Position.y && _Ball.Position.y < _Blocks[i].Position.y + _Blocks[i].Height) {
